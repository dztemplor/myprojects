...
int scullp_vma_fault1(struct vm_area_struct *vma,
                                struct vm_fault *vmf)
{
    ...
    {
        //address returned from ioremap() can't be called with virt_to_page (not low memory), virt_addr_valid returns false
    vaddr = ioremap_nocache(0x50410000, 4096);
    //works, the normal kernel virtual address can be mapped to user space
    vaddr = kmalloc(4096, GFP_KERNEL);
    //need call vmalloc_to_page to get the correct page.
    vaddr = vmalloc(4096);
    }
    pageptr = vaddr;

	if (!pageptr) goto out; /* hole or end-of-file */
    printk("%p %d\n", vaddr, virt_addr_valid(vaddr));
